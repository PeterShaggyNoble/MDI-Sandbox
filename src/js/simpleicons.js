(async()=>{
	...
	for(key in simple)
		if(simple.hasOwnProperty(key)){
			...
			if(icons[key].retired)
				use.setAttribute(`href`,icons[key].retired>version?`#deprecated`:`#rejected`);
			else{
				use.setAttribute(`href`,icons[key].added&&icons[key].added>version?`#added`:`#available`);
			}
			svg.classList.add(use.getAttribute(`href`).slice(1));
			svg.append(use);
			td.append(svg);
			tr.append(td);
			td=td.cloneNode(0);
			svg=svg.cloneNode(0);
			svg.classList.remove(use.getAttribute(`href`).slice(1));
			svg.classList.add(simple[key].status);
			use=use.cloneNode(0);
			...
			td=td.cloneNode(0);
			if(simple[key].note)
				td.append(document.createTextNode(simple[key].note));
			tr.append(td);
			body.append(tr)
		}
	for(key of legends)
